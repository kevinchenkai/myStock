...
- `tests/test_pnl.py`：交易盈亏（移动平均成本、成本兜底、超卖未覆盖、乱序处理）、单股复盘（FIFO 配对、胜率/盈亏比、持有天数、客观观察等）与年度财务统计（现金流口径、按市场分组、可选年份集合）。
- `tests/test_db.py`：UPSERT 幂等、当天行情覆盖、持仓快照覆盖、代码全集去重、跳过名单重置（全量/指定）、代码彻底清除（`purge`）。
- `tests/test_yf_client.py`：yfinance `history` 的 `end` 排他修正（`_end_inclusive`：end+1 天、None/异常格式透传）与限频退避（429 识别、限频时更长退避）。
- `tests/test_futu_funds.py`：账户资金规整（`'N/A'` → None、综合账户单行拆分）与盘面快照字段提取。
- `tests/test_capital_flow.py`：资金流向规整（日期截取、`'N/A'` 处理、缺列不报错）与 `capital_flow` UPSERT 幂等 / `purge` 清理。
