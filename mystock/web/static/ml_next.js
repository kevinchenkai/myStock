...
function renderReview(){$('review').innerHTML=[...reviewRows.entries()].reverse().map(([i,r])=>{const p=r.prediction,d=r.daily;return `<tr><td><button type="button" data-row="${i}">${esc(r.date)}</button></td><td>${esc(p?.as_of)}</td><td>${esc(labels[r.status]||r.status)}${r.hourly_sources?.includes('futu_none')?'<br><small class="muted">小时线：Futu · 不复权</small>':''}</td><td>${esc(r.prediction_status)}</td><td>${num(p?.l_hat)} / ${num(p?.h_hat)}</td><td>${num(d?.low)} / ${num(d?.high)}</td><td>${r.hit==null?'—':r.hit?'是':'否'}</td></tr>`;}).join('');$('review').querySelectorAll('button').forEach(b=>b.addEventListener('click',()=>selectDate(Number(b.dataset.row))));mountChart();$('orders').textContent='未选中日期。';$('selected').textContent='风险边界不等于固定政策的可执行报价。';}
