...
async function load(simulate=false){if(controller)controller.abort();controller=new AbortController();const signal=controller.signal,id=++seq,p=params();$('status').className='';$('status').textContent='正在读取隔离数据库…';
 try{const tasks=[get('/api/ml/v2/latest?source='+p.get('source'),signal),get('/api/ml/v2/review?'+p,signal)];if(simulate)tasks.push(get('/api/ml/v2/compare?'+p,signal));const result=await Promise.all(tasks);if(id!==seq)return;
 ...
 reviewRows=result[1].results[0].rows;comparison=simulate?result[2]:null;renderReview();renderCompare();$('status').textContent=`${p.get('codes')} · ${reviewRows.length} 个市场 session · ${simulate?'模拟完成':'行情复盘已载入；填写参数后模拟'}`;
...
function renderReview(){$('review').innerHTML=[...reviewRows.entries()].reverse().map(([i,r])=>{const p=r.prediction,d=r.daily;return `<tr><td><button type="button" data-row="${i}">${esc(r.date)}</button></td><td>${esc(p?.as_of)}</td><td>${esc(labels[r.status]||r.status)}</td><td>${esc(r.prediction_status)}</td><td>${num(p?.l_hat)} / ${num(p?.h_hat)}</td><td>${num(d?.low)} / ${num(d?.high)}</td><td>${r.hit==null?'—':r.hit?'是':'否'}</td></tr>`;}).join('');$('review').querySelectorAll('button').forEach(b=>b.addEventListener('click',()=>selectDate(Number(b.dataset.row))));mountChart();$('orders').textContent='未选中日期。';$('selected').textContent='风险边界不等于固定政策的可执行报价。';}
