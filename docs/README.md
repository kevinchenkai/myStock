...
2026-09-06 只读调研后，用户授权在 mk 安装 catboost／xgboost／arch 并在冻结副本上执行首轮预注册矩阵；结果为负，无候选过门槛，生产模型不变。随后的误差归因指向隔夜跳空，港股“时点前移＋隔夜信息”经 D4 验证，腾讯、阿里通过门槛；美股以盘前价为特征的同机制 D4 亦通过（英伟达、特斯拉）；工程接入与 D5 shadow 待决定。
