...
| 调度 | 建立真实调度（launchd 或 cron），按市场分两个时点：美股收盘后（≥16:30 ET）、港股收盘后（≥16:30 HKT）；`ml.sh` 注释与 README 按实际写 | `ml_sync_log` 连续 5 个交易日有准点记录 |
...
**剩余待用户拍板**：① 调度用 launchd 还是 cron，两个市场各自的触发时点；② 策略层本金 / 每次数量 / 库存上限 / 费用表的初值（Codex §0.4 待定项）；③ 第三批是否包含跨标的池化（E6），还是等 E1–E5 结果再定。
