...
打开 [本地预览](http://127.0.0.1:8896/ml-next)，回溯默认选择“历史回溯（包含离线重建）”，可直接选择 20/60/120。复盘明细会直接读取数据库，无需预生成页面；填写场景后点击“重新模拟当前窗口”才计算策略。下一目标日卡片及 API 默认 live 模式继续排除重建版本。
...
数据补齐后页面仍出现“缺有效预测”的原因是刷新后默认过滤为 live，并非 720 条重建记录丢失。回溯区域现默认包含历史重建，来源明确显示“历史重建”；下一目标日卡片固定请求 live，避免把回溯结果当成下一日预测。API 默认仍为 live。

仅看 live 时，后端额外返回 `has_recomputed`，页面把“已有历史重建但未选用”与“确实无当时预测”区分，并提供一键切回历史模式。状态栏显示实际可复盘天数；`/ml-next` 与 `/ml-next/` 均可打开。本次未采集或重算数据库，211 项测试通过，浏览器验证默认加载、live 筛选提示、一键切换和 120 日窗口。
