...
> 作者：Codex · 日期：2026-09-05 · 基线 `b2a2e155095079e0e2aa76c5d4e61aef8248bd0c` / `main`。状态：完成代码与官方接口文档只读调研，供功能讨论使用；用户已要求暂不派发 Astra 或直接执行。
...
讨论稿（暂不执行）：[Web 数据可用性升级工单](../records/web-data-upgrade-work-order_codex_20260905.md)。此前 [Futu 调研](futu-api-research_claude_20260718.html) 的账户资金、盘面增量字段和资金流已在代码中落地，本轮不重复列作新功能。
...
用户在工单派发前明确改为“先查依赖是否更新，先讨论功能，不直接执行”。未向 Astra 发送工单，也未安装包、改代码或启动部署。
...
上述为官方文档与本地 SDK 的静态证据，行情权限、股票覆盖及 SDK/OpenD 组合仍需后续独立验证。依赖升级建议先在隔离环境做导入、mock 回归及有限公开行情探测，确认后再安排运行环境升级；当前讨论阶段不执行这些操作。
