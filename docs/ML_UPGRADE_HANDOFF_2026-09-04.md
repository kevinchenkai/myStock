...
当前预览：[ML v2](http://127.0.0.1:8896/ml-next)。默认只读有时间证据的 live；当前旧库没有这类有效预测，显示 unavailable 是正确结果。选择“包含离线历史重建”，再点击“填入合成测试场景”可审查已有模型的固定策略闭环。该按钮写入浏览器表单，不写任何账户/数据库。20/60/120 为独立初始状态的重模拟；每日明细是该次连续回放的切片。
