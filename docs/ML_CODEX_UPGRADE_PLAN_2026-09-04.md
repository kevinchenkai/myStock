...
> 作者：Codex · 调研／更新日期：2026-09-04（America/Los_Angeles）· 版本：v1.2
>
> 状态：只读调研后的实施提案，尚未实施模型或业务改动。
> 代码基准：`beefd0c`（2026-09-03）；v1.0 首次调研开始时 Git 工作区干净。
> 本轮交付仅本文件。数据库通过 SQLite `mode=ro` 读取，统计连接另设 `query_only=ON`；未抓取行情、重训生产模型、回填预测、生成业务报告、发布或操作交易。
>
...
7. 冻结最近 120 个 session 作为工程 holdout 之前，核查是否已被旧实验调参使用。由于历史团队已观察过这些行情，不能宣称它是从未看过的锁箱。正式上线证据需额外积累协议冻结后的前向 shadow 样本。
...
特征扩充可在 E5 通过后再加入市场指数、财报事件等外部数据。美港市场的时区必须以 `available_at` 做 as-of join，例如香港收盘预测不能使用之后才完成的当日美股收盘。资金流数据可读已有生产快照，但历史较短、修订与公布时间需先核验；不得把当前 profiles 中的估值、交易单位或事件信息无日期地回填到历史。
...
采用增量建表和兼容读取，初版不删除旧 `ml_predictions`：
...
订单、成交、账本先实现为纯函数返回的结构；是否持久化逐事件表按复盘体积决定，不为了架构完整度先建大批空表。
...
保留旧 `GET /api/ml/strategy` 的兼容返回，新增清晰版本的只读 API，避免悄悄改变旧收益口径：
...
按可独立审查的变更拆分，先小范围完成闭环。以下为单开发执行的粗估，不包含真实市场 shadow 等待时间；实施时每个任务先检查最新代码与数据状态。

| 阶段 | 交付及主要文件 | 验收／停止条件 | 估算 |
| --- | --- | --- | --- |
| A：可信读取与缺口 | `db.py / data.py / strategy.py / review.py / web/app.py`；只读连接、日历窗口、来源／缺口状态、参数校验；补 API 测试 | 数据库不可写；缺一天仍显示一天缺口；不存在静默股票替换／重复汇总 | 2–3 天 |
| B：留档版本与管线状态 | `schema.sql / db.py / report.py / backfill.py / scripts/ml.sh`；新增版本表、兼容迁移、运行 manifest、失败状态与发布新鲜度 | 重跑不覆盖已发布预测；旧记录不伪装 live；单步失败最终非零；报告失败不推进 latest | 2–3 天 |
| C：统一评估与基线 | `baselines.py / evaluation.py / experiments.py`（新增）、`predictor.py / cv.py / signal_eval.py`；复现 naive_vol 对照、固定 OOF、时间截止、尾折补齐、raw/cal 指标分离 | 所有未来可用时刻断言通过；同输入同协议复现；可对照现有基线 | 3–4 天 |
| D：小模型实验 | `features.py / predictor.py / calibrator.py / config.py`；按 E1→E5 执行消融，E6–E8 视结果立项 | 第 6.4 节门槛；无改善就保留 E0，记录负结果 | 3–5 天 |
| E：共享账户撮合 | `execution.py`（新增）＋`simulator.py / strategy.py / backtest.py`；库存状态机、报价、约束、费用、事件账本、基线 | 手算路径完全一致；现金／库存守恒；无裸空；未完成回合不丢弃 | 3–4 天 |
| F：页面联动 | v2 API、`app.js / style.css / index.html`；下一目标日卡片、来源筛选、双层指标、同价格尺度图表、单日解释、URL 场景 | 同预测 ID 可追到报价与账本；异常和竞争场景通过；性能基线达标 | 2–3 天 |
| G：shadow 与默认切换 | 新旧运行并存、报告读取同一评估、配置回滚 | 前向样本足够且离线／线上口径一致；预测和策略各自通过门槛后分别切换 | 1–2 天工程＋至少 60 个市场 session 观察 |

总工程粗估 **16–24 人日**。A/B 可先交付可用性修复；C 是任何准确性宣称的前置；E/F 可以围绕冻结的旧模型建设，不需要等待候选模型胜出。即使 D 没有胜者，也不阻塞受约束回溯交付。

v1.1 新增项按范围管理：naive_vol、简单库存状态机、下一目标日卡片纳入核心；财报／指数采集、E6–E8 模型、完整真实订单假设回放和大规模物化均为扩展。核心工期仍是粗估，扩展另计，不能把新增研究工作无成本塞入原排期。模型训练与研究只通过离线任务运行；Web 不触发模型更新或下单。

首个可审查任务建议为 A：它直接修复当前观察到的预测缺口和 API 误导问题，改动较小，也为后面所有评分提供可信日期范围。B 中版本化完成前，不应开始会替换历史留档的批量实验。

## 10. 验证矩阵与回滚

### 10.1 本轮实际测试

本节为 v1.0 的独立验证记录；v1.1–v1.2 仅核对文档、相关源码、只读数据、数学关系及参考资料，未重新跑测试或训练模型。文档迭代检查包括本地引用有效性、代码围栏配对和变更范围。
