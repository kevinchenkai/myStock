...
> 作者：Codex · 调研／更新日期：2026-09-04（America/Los_Angeles）· 版本：v1.3
>
...
3. “发布时间”表示预测向指定消费端实际可用的时刻，与生成／导入时间分开。初版仅将完成且有效的预测事务性写入可见版本表；失败运行留在 manifest。旧记录不知道的时间保留 NULL／unknown。
...
| --- | --- | --- | --- |
| 第一批：可用性与数据边界 | `db.py / data.py / strategy.py / review.py / fetch.py / backfill.py / web/app.py`；Web 只读连接、日历窗口、缺口状态、参数校验；行情 inf／高低关系、db_path 透传；按证券核实交易规则与策略数量；核实调度后更新 `scripts/ml.sh` 时区说明 | Web 写 SQL 必失败；离线回填可向指定测试 ML 库写入；缺一天显示一天；无静默股票替换／重复汇总；坏 OHLC 被拒绝 | 约 3 天 |
...
按上述收缩范围，总工程粗估 **18–24 人日＋前向观察**，替代 v1.2 的 16–24 人日粗估。第二批不能为了压到三天省略不可覆盖写入与截止测试。多分位／池化、外部特征采集、完整真实订单假设回放、大规模物化均为扩展，另行估算。
...
本节为 v1.0 的独立验证记录；v1.1–v1.3 仅核对文档、相关源码、只读数据、数学关系及参考资料，未重新跑业务测试或训练模型。文档迭代检查包括本地引用有效性、代码围栏配对、diff 空白检查和变更范围。
