...
## 6. 修订附注（2026-09-06 v1.1，按 Codex 审核 P02）

- §2 "`available_at` 等于截止 → 不进入特征"原实现返回 0.0（被当成美股假日），现改为 NaN（数据延迟，失败关闭）；原测试断言已相应改正。拼接规则改为按美股日历重建预期序列：任一预期交易日或起始前一日缺行、价格无效或未及时可用 → NaN；仅日历证明无预期交易日才记 0；收益用各行自身 close，不跨缺失日。
- §3 的后移检验只说明结果依赖正确对齐，不能替代实际决策截止、供应商可用时间与数据版本检查。
