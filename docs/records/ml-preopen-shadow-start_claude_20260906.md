...
## 5. 修订附注（2026-09-06 v1.1，按 Codex 审核 P01／P03／P05）

- §3 的港股首条记录由修订前代码生成（冻结时钟、无决策上界、V1／V2 未强制配对），归为**工程验证记录，不计入正式 D5**；保留在版本表不删改，`shadow_report` 以 `protocol` 字段区分。
- 修订后的控制流：采集前逐码守卫 → 两源同时采集并按事件时间校验 → `decision_at` 作为拼接上界 → 生成完成后再守卫，超时不入档 → 只入档 V1／V2 配对；每次尝试必写回执，行内记录输入哈希、实际后端、代码提交与协议版本。
- §2 "窗口外运行会按状态拒绝"现在对采集之前与生成之后都成立。
- `shadow_report` 规则：成熟判定用 `daily_final`，每目标日取最早完整配对，行内 alpha，B1 重建，计划日分母与尝试状态分布。详见 [修订回执](ml-preopen-review-fixes_claude_20260906.md)。
