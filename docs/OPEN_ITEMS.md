...
> 最后更新：2026-09-06（Codex 审核 P01–P08／S01–S04 已修订，见修订回执；正式 D5 自修订后首条 shadow 起算；生产默认仍为 V1）。
...
| 新一轮：开盘前决策＋隔夜／盘前信息（两市） | 两市 D4 主力通过筛选门槛并经 Codex 审核修订（[修订回执](records/ml-preopen-review-fixes_claude_20260906.md)：加同容量对照、逐股区间、每块选参重跑）；shadow 控制流已按 P01–P05 重做 | 进行中：正式 D5 自修订后首条 shadow 起算，每市 ≥60 计划日、主力逐股 ≥60 合规配对；生产继续 V1；切换门槛见 Codex 审核 §7 |
...
## PREOPEN-REVIEW-20260906（Codex 审核，已修订）

来源：[ML 开盘前建模与 shadow 审核](research/ml-preopen-review_codex_20260906.md) §3–§7，审核基线 `6da6d6c`；处置见 [修订回执](records/ml-preopen-review-fixes_claude_20260906.md)。

| 编号 | 事项 | 状态 |
| --- | --- | --- |
| P01 | 真实时钟贯穿采集／生成／入档，特征受实际决策上界限制；港股长假等待完整美股窗口 | ✅ 已关闭（修订回执 §0） |
| P02 | 延迟 ADR 不得记零；按美股日历验证收益分母、完整序列和有效价格 | ✅ 已关闭（含测试断言改正） |
| P03 | 校验盘前事件时间／新鲜度；逐股处理空／部分失败，先验有效性再选主备源并记录实际来源 | ✅ 已关闭 |
| P04 | 严格核对 08:00–09:00 bar；解释"730 天"与 2023-10-09 起、726 行的矛盾 | ✅ 已关闭：730 个交易日、726 个有 08:00 bar |
| P05 | 双源同时点对照、不可变输入引用、真实后端、成熟／配对评分、B1 与失败分母 | ✅ 代码已关闭；分层（普通日／跳空日／公司行动日）与逐来源误差对照待样本累积后实现 |
| P06 | 工具矩阵跨块选参复用早期校准段；两侧评分协议 | ✅ 已关闭：每块选参并重跑树模型组（修订回执 §3） |
| P07 | KWEB 需匹配基线与日期；观察组"主判定通过"及主力置信区间表述 | ✅ 已关闭（重跑＋附注） |
| P08 | 撤回 ±3% 天花板／排除学习器瓶颈；特征＋容量组合说明 | ✅ 已关闭：附注撤回，D4 加同容量对照 O3 |
| S01 | D4 与 shadow 的协议对照 | ✅ 已关闭（修订回执 §4） |
| S02 | W2 证据、逐股日期与实际同日配对区间 | ✅ 已关闭（修订回执 §2） |
| S03 | GARCH 非零收敛标志、全回退／候选失败路径与 D4 逐行留档 | 部分完成：收敛拒绝与逐行留档已做；矩阵全回退／整候选失败汇总路径待办 |
| S04 | V1 基线结果回归与 mock 时钟／数据边界测试 | ✅ 已关闭（`b988ea2` 冻结期望值） |
| SH-01 | 港股首条 shadow（2026-09-06 14:03 UTC）归为工程验证记录，不计入正式 D5；正式 D5 自修订后首条起算 | 已判定 |
