...
> 最后更新：2026-09-06（Codex 审核 `a8fa729..6da6d6c`，新增开盘前建模／shadow 的 P01–P08、S01–S04；本次只提交审查与导航，未修复实现或切换生产模型）。
...
| 前向 shadow 未开始（原阶段状态） | 历史重建**不是** live、也不是前向 shadow；2026-09-06 港股首条已由执行方留档，见下行 | 已进入新阶段；正式 D5 有效样本认定与闭环修订仍待办，见 PREOPEN-REVIEW-20260906 |
...
| 新一轮：开盘前决策＋隔夜／盘前信息（两市） | 两市 D4 回执报告主力通过 raw pinball 筛选（[港股](records/ml-overnight-d4_claude_20260906.md)、[美股](records/ml-preopen-us-d4_claude_20260906.md)）；工程接入并留档港股首条（[回执](records/ml-preopen-shadow-start_claude_20260906.md)）；[Codex 新审核](research/ml-preopen-review_codex_20260906.md) 已完成 | 进行中：P01–P05 修订前，现有报告不能作 D5 晋级依据；每市 ≥60 session 需逐股合规配对、成熟判定和缺失分母，生产继续 V1；页面标注待实际切换时完成 |
...
## PREOPEN-REVIEW-20260906（Codex 审核待回应）

来源：[ML 开盘前建模与 shadow 审核](research/ml-preopen-review_codex_20260906.md) §3–§7，审核基线 `6da6d6c`。沿用审查编号；执行方应另立修订回执，不覆盖原实验正文。本表是待办登记，不自动授权新的训练、运行库操作或上线。

| 编号 | 事项 | 状态 |
| --- | --- | --- |
| P01 | 真实时钟贯穿采集／生成／入档，特征受实际决策上界限制；港股长假等待完整美股窗口 | 待办，合规 D5 样本认定前修订 |
| P02 | 延迟 ADR 不得记零；按美股日历验证收益分母、完整序列和有效价格 | 待办，含现有错误测试断言 |
| P03 | 校验盘前事件时间／新鲜度；逐股处理空／部分失败，先验有效性再选主备源并记录实际来源 | 待办 |
| P04 | 严格核对 08:00–09:00 bar；解释美股“730 天”与 2023-10-09 起、726 行的原始证据矛盾 | 待办，需执行方查原冻结输入 |
| P05 | 双源同时点对照、不可变输入引用、真实后端、成熟／配对评分、B1 与失败分母 | 待办，现有 shadow_report 不作晋级依据；后端问题同时关联原 R-04 |
| P06 | 工具矩阵跨块选参复用早期校准段；联合两侧评分与当前每侧独立实现不一致 | 待办，先补集合核对与协议修订，不能未经证据声称测试标签已泄漏 |
| P07 | KWEB 需匹配基线与日期；观察组“主判定通过”及主力置信区间表述需修订 | 待办，不直接否定四只主力 raw 数值 |
| P08 | 撤回 ±3% 天花板／排除学习器瓶颈；准确说明特征＋容量组合及后移检验局限 | 待办，另立回应保留旧正文 |
| S01 | 对齐 D4 与 shadow 的事件掩码、训练史、重拟合频率与 250 行下限口径 | 建议修订；生产等价性在切换前必须验证，关联原 R-03 |
| S02 | W2 的选择／使用证据、逐股日期与实际同日配对区间 | 建议修订，不把五种子／相邻窗口当独立样本 |
| S03 | GARCH 非零收敛标志、全回退／候选失败路径与 D4 逐行预测留档 | 建议修订；回执本轮零失败，未确认影响历史数值 |
| S04 | V1 基线结果回归与 mock 时钟／数据边界测试 | 建议修订，现有“unchanged”测试未比较旧输出 |
