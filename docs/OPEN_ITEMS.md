...
| 新一轮：开盘前决策＋隔夜／盘前信息（两市） | 港股与美股 D4 均过门槛（[港股](records/ml-overnight-d4_claude_20260906.md)、[美股](records/ml-preopen-us-d4_claude_20260906.md)）；工程接入完成并启动 shadow（[回执](records/ml-preopen-shadow-start_claude_20260906.md)） | 进行中（D5 shadow）：每个交易日人工运行 `ml.sh shadow HK`（08:30 HKT）与 `ml.sh shadow US`（09:00 ET），累计 ≥60 session 后用 `shadow_report` 评估；生产不切换；五份回执待交 Codex Astra 审核；页面"开盘前预测"标注待切换时一并做 |
