...
生成应在所需市场收盘数据最终确认后、下一目标 session 决策截止前；盘中/缺最终数据/过截止时跳过是正确行为。US 截止为当地 09:30 ET，HK 为 09:00，不能用所有市场一个固定本地时刻推定可运行。`ml.sh` 无参数只显示帮助。`publish`/`all` 会触发公网发布，不是本地 Web 更新的必要步骤。

需要公网报告时，在 train 后运行它打印的命令，例如 `bash scripts/ml.sh publish data/ml/receipts/<train-run-id>.json`（替换真实路径）。可换终端；不自动选择最近回执。也可显式设置 `MYSTOCK_ML_RECEIPT`，已设置的 `MYSTOCK_ML_RUN_ID` 必须匹配。校验状态、文件哈希和目标截止后仅覆盖公网 `index.html`，日期归档不上传；本地历史在 `reports/runs/<run-id>/`。`.data.json` 数据回执不可发布。任一采集失败仍使 `all` 停止，该限制见修复回执待办。
...
`ml-next` 复用 `style.css`、`theme.js` 及与首页共用的页头/首屏主题模板。卡片、表格、按钮、字体和红涨绿跌使用同一套变量；K 线主题切换原地更新，保留选中日期与价格叠加线。主题按钮禁止窄屏拆字。当前回归：211 项通过；另检查 JavaScript/脚本语法、首页模板、深浅主题、390px 手机布局及合成场景回放。具体提交见分支最新日志。
